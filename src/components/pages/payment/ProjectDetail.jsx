...
const ProjectDetail = (props) => {
  ...
  const getPaymentDateText = (subscription) => {
    ...
    if (subscription.is_default) {
      return DateFormat(subscription.expiring_date, i18n.language);
    }
    if (subscription.payment_overdue_days !== null) {
      ...
      return DateFormat(subscription.payment_date, i18n.language);
    }
    return DateFormat(subscription.payment_date, i18n.language);
  };
    ...
    <TabContent>
      ...
      >
        ...
        <div className="intro-y text-gray-500 text-sm my-4 mt-1">
          {t('created')}: {DateFormat(project.created_at, i18n.language)}
        </div>
                ...
                <tbody>
                  {project.invitations.map((invite) => (
                    <tr key={invite.email}>
                      ...
                      <td className="border-b w-1/3">{DateFormat(invite.updated_at, i18n.language)}</td>
            ...
            <tbody>
              {project.subscriptions.map((subscription) => (
                ...
                >
                  ...
                  <td className="border-b">
                    {DateFormat(getPaymentDateText(subscription), i18n.language)}
                  </td>
                  {project.is_owner && (
                    <td className="border-b">
